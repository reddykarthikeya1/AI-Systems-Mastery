"""Global pytest root configuration for all 27 modules."""
...
if cwd.name == "starter" or "starter" in cwd.parts:
    ...
    if starter_dir.parent.name == "Module_26_Final_Capstone_Project" and "capstone_platform" not in sys.modules:
        pkg = types.ModuleType("capstone_platform")
        pkg.__path__ = [str(starter_dir)]
        sys.modules["capstone_platform"] = pkg
else:
    for p in root_dir.rglob("*"):
        if (
            p.is_dir()
            and (p.name in {"src", "project_solution", "capstone_platform"} or p.name.startswith("Module_"))
