"""Reference solution - Problem 02: Which Edges Are the Bottleneck?
...
def min_cut_edges(nodes: int, edges: list[tuple[int, int, int]],
    ...
    graph, to, cap = _build(nodes, edges)
    _max_flow(graph, to, cap, source, sink)
    ...
    while queue:
        ...
        for edge_id in graph[node]:
            nxt = to[edge_id]
            if not reachable[nxt] and cap[edge_id] > 0:
                reachable[nxt] = True
                queue.append(nxt)

    crossing = []
    for a, b, c in edges:
        if c > 0 and reachable[a] and not reachable[b]:
            crossing.append((a, b))
