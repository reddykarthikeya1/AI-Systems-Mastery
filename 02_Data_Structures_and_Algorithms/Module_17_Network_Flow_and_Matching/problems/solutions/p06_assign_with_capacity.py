"""Reference solution - Problem 06: Workers Who Can Take Several Shifts
...
def staff_shifts(workers: int, shifts: int, available: list[tuple[int, int]],
                 shifts_each: int) -> int:
    source = workers + shifts
    sink = source + 1
    edges = [(source, w, shifts_each) for w in range(workers)]
    edges += [(workers + s, sink, 1) for s in range(shifts)]
    edges += [(w, workers + s, 1) for w, s in available]
