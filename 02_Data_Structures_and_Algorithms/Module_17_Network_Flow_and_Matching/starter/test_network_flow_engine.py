...
def test_flow_disconnected_source_sink_edge_cases():
    fn = FlowNetwork(4)
    # No edges: max flow is 0
    assert dinic(fn, 0, 3) == 0
    assert edmonds_karp(fn, 0, 3) == 0
    
    # Path from 0 to 1, and 2 to 3, but disconnected between 1 and 2
    fn.add_edge(0, 1, 10)
    fn.add_edge(2, 3, 10)
    assert dinic(fn, 0, 3) == 0


def test_bipartite_matching_edge_cases():
    # Empty bipartite matching
    assert bipartite_matching(0, 0, []) == []
    
    # 2 left nodes, 2 right nodes, isolated matching
    res = bipartite_matching(2, 2, [(0, 0), (1, 1)])
