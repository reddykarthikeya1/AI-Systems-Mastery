...
def test_graph_empty_and_isolated_vertices():
    ge = GraphEngine[str]()
    assert ge.count_connected_components() == 0
    
    # Isolated vertices
    ge.add_vertex("A")
    ge.add_vertex("B")
    ge.add_vertex("C")
    assert ge.count_connected_components() == 3
    assert ge.bfs("A") == ["A"]
    assert ge.dfs("B") == ["B"]


def test_graph_topological_sort_cycle_edge_cases():
    ge = GraphEngine[str]()
    ge.add_edge("A", "B")
    ge.add_edge("B", "C")
    ge.add_edge("C", "A")  # Cycle!
