...
def test_kruskal_empty_and_disconnected():
    ...
    assert weight == 0.0
    assert edges == []
def test_dijkstra_unreachable_and_self_edge_cases():
    engine = ShortestPathMSTEngine[str]()
    engine.add_edge("A", "B", 5.0)
    engine.add_edge("C", "D", 2.0)
...
def test_kruskal_empty_and_disconnected_edge_cases():
    engine = ShortestPathMSTEngine()
    cost, edges = engine.kruskal_mst()
    assert cost == 0.0
    assert edges == []
    
    # Single edge
    engine.add_edge("X", "Y", 10.0)
    cost, edges = engine.kruskal_mst()
    assert cost == 10.0
    assert len(edges) == 1
