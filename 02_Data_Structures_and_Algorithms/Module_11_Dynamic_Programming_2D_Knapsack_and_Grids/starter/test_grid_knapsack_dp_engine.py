...
def test_knapsack_zero_capacity_and_oversized_edge_cases():
    dp = GridKnapsackDPEngine()
    # Zero capacity returns (0, [])
    assert dp.knapsack_01([5, 10], [10, 20], 0) == (0, [])
    # Empty items returns (0, [])
    assert dp.knapsack_01([], [], 50) == (0, [])
    # All items heavier than capacity
    assert dp.knapsack_01([100, 200], [10, 20], 50) == (0, [])


def test_lcs_and_edit_distance_edge_cases():
    dp = GridKnapsackDPEngine()
    # LCS with empty strings
    assert dp.longest_common_subsequence("", "abc") == 0
    assert dp.longest_common_subsequence("abc", "") == 0
    assert dp.longest_common_subsequence("abc", "abc") == 3
