...
def test_merge_intervals_empty_and_single_edge_cases():
    eng = IntervalSchedulingEngine()
    assert eng.merge_intervals([]) == []
    assert eng.merge_intervals([[1, 5]]) == [[1, 5]]
    # Duplicate intervals
    assert eng.merge_intervals([[1, 3], [1, 3]]) == [[1, 3]]
    # Touching intervals
    assert eng.merge_intervals([[1, 2], [2, 3]]) == [[1, 3]]


def test_meeting_rooms_and_overlap_edge_cases():
    eng = IntervalSchedulingEngine()
    assert eng.min_meeting_rooms([]) == 0
    assert eng.min_meeting_rooms([[1, 5]]) == 1
    # Non-overlapping
    assert eng.min_meeting_rooms([[1, 2], [2, 3], [3, 4]]) == 1
    # Completely nested
    assert eng.min_meeting_rooms([[1, 10], [2, 8], [3, 6]]) == 3
    # Erase overlaps
    assert eng.erase_overlap_intervals([]) == 0
    assert eng.erase_overlap_intervals([[1, 2], [2, 3]]) == 0
    assert eng.erase_overlap_intervals([[1, 2], [1, 2], [1, 2]]) == 2
