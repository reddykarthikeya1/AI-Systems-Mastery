...
- **Affected Subsystem:** Module 20 AI Vector Databases pgvector Qdrant
...
1. Navigate to this module's debug lab:
   ```bash
   cd Module_20_AI_Vector_Databases_pgvector_Qdrant/debug_lab
   ```
2. Run the repro script:
   ```bash
   python broken_vector_distance.py
   ```
3. Observe the crash or invariant failure.

### 🎯 Your Objective
1. Inspect `broken_vector_distance.py` to identify the root cause.
