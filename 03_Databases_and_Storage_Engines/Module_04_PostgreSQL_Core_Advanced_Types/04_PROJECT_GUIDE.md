# Module 04 PostgreSQL Core Advanced Types: Project Guide

## 📌 Project Overview: Poly-Schema Catalog with JSONB, Arrays & Custom Domains

This comprehensive guide walks you step-by-step through building the project for **PostgreSQL Core Architecture & Advanced Types**.
You will implement both the **internal algorithmic mechanics** (Track A) and **production-grade live operations** (Track B) utilizing **PostgreSQL native types, GIN indexing, and connection pooling**.
...
| :---: | :--- | :--- | :--- |
| **Tier 1** | 🟢 **Scaffolded Beginner** | Fundamentals & initial setup | Create standard relational tables with text, integer, and timestamp fields. |
| **Tier 2** | 🟡 **Core Production Project** | Standard course completion | Implement a schema-agnostic product catalog supporting JSONB document containment queries, tag array intersections, and connection pooling via psycopg2 ThreadedConnectionPool. |
| **Tier 3** | 🔴 **Architect Stretch Challenge** | Mastery & systems engineering | Construct custom composite types and range types with EXCLUDE USING GIST constraints preventing double-booking anomalies. |
...
- [ ] All Track A unit tests pass: `pytest Module_04_PostgreSQL_Core_Advanced_Types/project_solution/test_*_engine.py`
- [ ] Reconciliation tests pass without warnings: `pytest Module_04_PostgreSQL_Core_Advanced_Types/project_solution/test_*_live.py`
