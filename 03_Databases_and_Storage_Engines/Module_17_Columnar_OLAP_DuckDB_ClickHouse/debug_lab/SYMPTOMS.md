...
- **Affected Subsystem:** Module 17 Columnar OLAP DuckDB ClickHouse
...
1. Navigate to this module's debug lab:
   ```bash
   cd Module_17_Columnar_OLAP_DuckDB_ClickHouse/debug_lab
   ```
2. Run the repro script:
   ```bash
   python broken_clickhouse_writer.py
   ```
3. Observe the crash or invariant failure.

### 🎯 Your Objective
1. Inspect `broken_clickhouse_writer.py` to identify the root cause.
