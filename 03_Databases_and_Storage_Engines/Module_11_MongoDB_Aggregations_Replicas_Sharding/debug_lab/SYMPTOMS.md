...
- **Affected Subsystem:** Module 11 MongoDB Aggregations Replicas Sharding
...
1. Navigate to this module's debug lab:
   ```bash
   cd Module_11_MongoDB_Aggregations_Replicas_Sharding/debug_lab
   ```
2. Run the repro script:
   ```bash
   python broken_mongo_router.py
   ```
3. Observe the crash or invariant failure.

### 🎯 Your Objective
1. Inspect `broken_mongo_router.py` to identify the root cause.
