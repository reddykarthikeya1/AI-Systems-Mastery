# Module 13: Distributed Messaging & Event Streaming: Commit Logs
...
| **4** | **[02_interactive_commit_log_stream.ipynb](02_interactive_commit_log_stream.ipynb)** | Open in Jupyter/VS Code to run interactive visual experiments and benchmarks. |
| **5** | **[03_event_log_partitioning_demo.py](03_event_log_partitioning_demo.py)** | Run in terminal (`python 03_event_log_partitioning_demo.py`) to explore 03 Event Log Partitioning Demo code patterns. |
...
Append-only commit logs provide high-throughput sequential disk writes, partitioned event topics, and consumer group offset management with at-least-once replay.
...
```bash
jupyter notebook 00_interactive_commit_log_stream.ipynb
```

### 2. Execute the Standalone Demo Script
Observe the components interacting under simulated workload:
```bash
python 01_event_log_partitioning_demo.py
```
...
| :--- | :--- |
| **[`00_interactive_*.ipynb`](02_interactive_commit_log_stream.ipynb)** | Interactive hands-on simulation notebook with runnable visualization cells. |
| **[`03_event_log_partitioning_demo.py`](03_event_log_partitioning_demo.py)** | Standalone Python executable demonstrating core mechanics and latency profiles. |
...
| **[`project_solution/commit_log_stream.py`](project_solution/commit_log_stream.py)** | Verified, complete in-process architectural simulation reference model. |
| **[`starter/commit_log_stream.py`](starter/commit_log_stream.py)** | Student boilerplate raising `NotImplementedError` for TDD mastery. |
...
### ➡️ Next Steps
- [Next Module: Module 14 Distributed URL Shortener TinyURL](../Module_14_Distributed_URL_Shortener_TinyURL/01_README.md)
