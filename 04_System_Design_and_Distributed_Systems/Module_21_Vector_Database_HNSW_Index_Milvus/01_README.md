# Module 21: Vector Databases & Hierarchical Navigable Small World (HNSW) Indexes
...
| **4** | **[02_interactive_vector_database_engine.ipynb](02_interactive_vector_database_engine.ipynb)** | Open in Jupyter/VS Code to run interactive visual experiments and benchmarks. |
| **5** | **[03_vector_hnsw_search_demo.py](03_vector_hnsw_search_demo.py)** | Run in terminal (`python 03_vector_hnsw_search_demo.py`) to explore 03 Vector Hnsw Search Demo code patterns. |
...
Vector databases index dense high-dimensional embeddings using multi-layer HNSW graphs and 8-bit Scalar Quantization (SQ8) to achieve sub-10ms Approximate Nearest Neighbor (ANN) search.
...
```bash
jupyter notebook 00_interactive_vector_database_engine.ipynb
```

### 2. Execute the Standalone Demo Script
Observe the components interacting under simulated workload:
```bash
python 01_vector_hnsw_search_demo.py
```
...
| :--- | :--- |
| **[`00_interactive_*.ipynb`](02_interactive_vector_database_engine.ipynb)** | Interactive hands-on simulation notebook with runnable visualization cells. |
| **[`03_vector_hnsw_search_demo.py`](03_vector_hnsw_search_demo.py)** | Standalone Python executable demonstrating core mechanics and latency profiles. |
...
| **[`project_solution/vector_database_engine.py`](project_solution/vector_database_engine.py)** | Verified, complete in-process architectural simulation reference model. |
| **[`starter/vector_database_engine.py`](starter/vector_database_engine.py)** | Student boilerplate raising `NotImplementedError` for TDD mastery. |
...
### ⬅️ Prerequisites
- [Prerequisite: Module 12 Probabilistic Data Structures](../Module_12_Probabilistic_Data_Structures/01_README.md)
- [Prerequisite: Module 16 Social Media Newsfeed Recommendation TwoTower](../Module_16_Social_Media_Newsfeed_Recommendation_TwoTower/01_README.md)

### ➡️ Next Steps
- [Next Module: Module 22 Distributed LLM Serving PagedAttention vLLM](../Module_22_Distributed_LLM_Serving_PagedAttention_vLLM/01_README.md)

### 🏁 Phase Benchmark & Core Frameworks
- **[Relevant Phase Benchmark Checkpoint](../Phase_Checkpoints/PHASE_5_CHECKPOINT.md)**
