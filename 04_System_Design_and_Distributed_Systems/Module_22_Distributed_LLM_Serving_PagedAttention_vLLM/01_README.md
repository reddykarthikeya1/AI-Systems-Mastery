# Module 22: Distributed LLM Serving & PagedAttention (vLLM)
...
| **4** | **[02_interactive_llm_inference_engine.ipynb](02_interactive_llm_inference_engine.ipynb)** | Open in Jupyter/VS Code to run interactive visual experiments and benchmarks. |
| **5** | **[03_continuous_batching_kv_cache_demo.py](03_continuous_batching_kv_cache_demo.py)** | Run in terminal (`python 03_continuous_batching_kv_cache_demo.py`) to explore 03 Continuous Batching Kv Cache Demo code patterns. |
...
High-throughput LLM serving eliminates GPU memory fragmentation using PagedAttention non-contiguous block allocation, continuous iteration-level batching, and preemption.
...
```bash
jupyter notebook 00_interactive_llm_inference_engine.ipynb
```

### 2. Execute the Standalone Demo Script
Observe the components interacting under simulated workload:
```bash
python 01_continuous_batching_kv_cache_demo.py
```
...
| :--- | :--- |
| **[`00_interactive_*.ipynb`](02_interactive_llm_inference_engine.ipynb)** | Interactive hands-on simulation notebook with runnable visualization cells. |
| **[`03_continuous_batching_kv_cache_demo.py`](03_continuous_batching_kv_cache_demo.py)** | Standalone Python executable demonstrating core mechanics and latency profiles. |
...
| **[`project_solution/llm_inference_engine.py`](project_solution/llm_inference_engine.py)** | Verified, complete in-process architectural simulation reference model. |
| **[`starter/llm_inference_engine.py`](starter/llm_inference_engine.py)** | Student boilerplate raising `NotImplementedError` for TDD mastery. |
...
### ⬅️ Prerequisites
- [Prerequisite: Module 01 Physics of Scalability Capacity Math](../Module_01_Physics_of_Scalability_Capacity_Math/01_README.md)
- [Prerequisite: Module 21 Vector Database HNSW Index Milvus](../Module_21_Vector_Database_HNSW_Index_Milvus/01_README.md)

### ➡️ Next Steps
- [Next Module: Module 23 Distributed Transactions Sagas Outbox](../Module_23_Distributed_Transactions_Sagas_Outbox/01_README.md)
