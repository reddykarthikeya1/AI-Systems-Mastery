# Module 15: System Design Case Study: Real-Time Chat & Presence (Discord/Slack)
...
| **4** | **[02_interactive_chat_presence_platform.ipynb](02_interactive_chat_presence_platform.ipynb)** | Open in Jupyter/VS Code to run interactive visual experiments and benchmarks. |
| **5** | **[03_presence_heartbeat_demo.py](03_presence_heartbeat_demo.py)** | Run in terminal (`python 03_presence_heartbeat_demo.py`) to explore 03 Presence Heartbeat Demo code patterns. |
...
Real-time chat platforms manage persistent WebSocket sessions, ephemeral user presence tracking, distributed Pub/Sub fanout, and offline message mailbox spooling.
...
```bash
jupyter notebook 00_interactive_chat_presence_platform.ipynb
```

### 2. Execute the Standalone Demo Script
Observe the components interacting under simulated workload:
```bash
python 01_presence_heartbeat_demo.py
```
...
| :--- | :--- |
| **[`00_interactive_*.ipynb`](02_interactive_chat_presence_platform.ipynb)** | Interactive hands-on simulation notebook with runnable visualization cells. |
| **[`03_presence_heartbeat_demo.py`](03_presence_heartbeat_demo.py)** | Standalone Python executable demonstrating core mechanics and latency profiles. |
...
| **[`project_solution/chat_presence_platform.py`](project_solution/chat_presence_platform.py)** | Verified, complete in-process architectural simulation reference model. |
| **[`starter/chat_presence_platform.py`](starter/chat_presence_platform.py)** | Student boilerplate raising `NotImplementedError` for TDD mastery. |
...
### ➡️ Next Steps
- [Next Module: Module 16 Social Media Newsfeed Recommendation TwoTower](../Module_16_Social_Media_Newsfeed_Recommendation_TwoTower/01_README.md)
